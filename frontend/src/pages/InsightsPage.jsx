...
function computeFailureStats(runs) {
  ...
  for (const r of runs) {
    ...
    if (!byExperiment[exp]) byExperiment[exp] = { completed: 0, failed: 0, aborted: 0 };
  ...
  return Object.entries(byExperiment)
    .map(([exp, counts]) => ({
      ...
      failRate: counts.completed + counts.failed + counts.aborted > 0
        ? Math.round(((counts.failed + counts.aborted) /
            (counts.completed + counts.failed + counts.aborted)) * 100)
        : 0,
    }))
    .filter((d) => d.total > 0)
...
export default function InsightsPage() {
  ...
  const totalFailed    = runs.filter((r) => r.status === "failed" || r.status === "aborted").length;
  const totalRuns      = runs.length;
  const overallFailRate = totalRuns > 0 ? Math.round((totalFailed / totalRuns) * 100) : 0;
          ...
          {totalRuns > 0 && (
            ...
            >
              ...
              <div className="mb-4 grid grid-cols-3 gap-3">
                {[
                  { label: "Total runs",      value: totalRuns,          color: "text-espresso" },
                  { label: "Failed / Aborted", value: totalFailed,        color: "text-terracotta" },
                  { label: "Failure rate",    value: `${overallFailRate}%`,
                    color: overallFailRate > 30 ? "text-terracotta" : overallFailRate > 10 ? "text-amber-600" : "text-olive" },
                ].map(({ label, value, color }) => (
                  <div key={label} className="rounded-xl border border-line bg-sand/50 p-3 text-center">
                ...
                <>
                  ...
                  <ResponsiveContainer width="100%" height={Math.max(80, failureStats.length * 40)}>
                    <BarChart data={failureStats} layout="vertical" margin={{ left: 10, right: 50 }}>
                      ...
                      />
                      <Bar dataKey="failRate" radius={[0, 6, 6, 0]} maxBarSize={24}>
                        {failureStats.map((d, i) => (
                          <Cell key={i} fill={d.failRate > 50 ? TERRA : d.failRate > 25 ? AMBER : OLIVE} />
                        ))}
              ...
              {recentFailed.length > 0 && (
                <div className="mt-4">
                  <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Recent failures</div>
                  <div className="space-y-2">
                    {recentFailed.map((r) => {
                      const m = r.metrics || {};
                      const a = m.val_accuracy ?? m.val_acc ?? null;
                      return (
                        <div key={r.run_id} className="rounded-xl border border-terracotta/20 bg-terracotta/5 p-3">
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-muted">{r.experiment}</span>
                              </div>
                              {r.error_message && (
                              ...
                              {r.rationale && (
                                <div className="mt-0.5 line-clamp-1 text-xs text-muted">{r.rationale}</div>
                              )}
                            </div>
                            <div className="shrink-0 text-right">
                              <div className="text-xs font-semibold uppercase text-terracotta">{r.status}</div>
                              {a != null && <div className="text-xs text-muted">acc={a}</div>}
                            </div>
                          </div>
                        </div>
                      );
