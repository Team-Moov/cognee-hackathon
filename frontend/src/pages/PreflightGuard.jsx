...
const PRESETS = [
  {
    label: "ResNet50 + Adam (already tried!)",
    config: {
      model: "ResNet50",
      optimizer: "Adam",
      lr: 0.001,
  ...
  return (
    <div className="mx-auto w-full max-w-5xl p-6 sm:p-8 lg:px-10">
      ...
      <form
        ...
        className="space-y-4 rounded-2xl border border-line bg-card p-5 shadow-soft"
      >
        <div>
          <div className="mb-2 flex items-center justify-between">
            <label className="block text-xs font-semibold text-cocoa">
        ...
        <div className="fade-in mt-5">
          <div className="pulse-danger rounded-2xl border-2 border-terracotta/50 bg-terracotta/8 p-5">
            <div className="mb-3 flex items-center gap-3">
              ...
              <div>
                <div className="font-display text-lg font-semibold text-terracotta">
                  ALREADY TRIED
                </div>
                <div className="text-sm text-cocoa">
                  Exact config match found — do not waste compute.
                </div>
              </div>
            </div>
            ...
            <div className="mb-3 flex items-center gap-3">
              ...
              <div>
                <div className="font-display text-lg font-semibold text-ochre">
                  SIMILAR CONFIG FOUND
                </div>
                <div className="text-sm text-cocoa">
                  {(result.similarity_score * 100).toFixed(0)}% match — review
                  before running.
                </div>
              </div>
            </div>
        ...
        <div className="fade-in mt-5">
          <div className="rounded-2xl border-2 border-olive/40 bg-olive/8 p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-display text-lg font-semibold text-olive">
                  SAFE TO RUN
                </div>
                <div className="text-sm text-cocoa">
                  No matching or similar config in memory. Proceed.
                </div>
              </div>
            </div>
          </div>
        ...
        {mainMetric != null && (
          <span>
            ...
            <span className="font-mono text-espresso">
              {mainMetric.toFixed ? mainMetric.toFixed(3) : mainMetric}
            </span>
          </span>
        )}
        {run.gpu_hours != null && (
          <span>
            ...
            <span className="font-mono text-espresso">{run.gpu_hours}h</span>
          </span>
        )}
        {run.date && (
          <span>
