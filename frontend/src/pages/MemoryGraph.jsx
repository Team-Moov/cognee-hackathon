...
export default function MemoryGraph() {
    ...
    function resize() {
      if (wrapRef.current)
        setDims({
          ...
          h: Math.max(480, window.innerHeight - 220),
        });
    }
    resize();
  ...
  return (
    <div className="mx-auto w-full max-w-6xl p-6 sm:p-8 lg:px-10">
      ...
      <div
        ref={wrapRef}
        className="relative overflow-hidden rounded-2xl border border-line bg-card shadow-soft"
      >
        {loading ? (
          ...
          <div className="py-24 text-center text-sm text-muted">
            No memory in this project yet — record some runs.
          </div>
        ) : (
          <ForceGraph2D
            ref={fgRef}
        ...
        {selected && (
          <div className="absolute right-3 top-3 w-64 rounded-xl border border-line bg-card p-3 text-xs shadow-lift">
            <div className="mb-1 flex items-center justify-between">
              <span className="font-semibold uppercase tracking-wide text-coffee-deep">
                {selected.type}
              </span>
              <button
            ...
            {selected.status && (
              <div className="text-muted">status: {selected.status}</div>
            )}
            {selected.metric != null && (
              <div className="text-muted">metric: {selected.metric}</div>
            )}
            {selected.gpu_hours != null && (
              <div className="text-muted">{selected.gpu_hours}h GPU</div>
            )}
            {selected.config && (
              <code className="mt-1 block break-all text-[10px] text-cocoa">
                {JSON.stringify(selected.config)}
              </code>
            )}
            {selected.type === "run" && (
              <button
                onClick={() => nav(`/lineage/${selected.id}`)}
                className="mt-2 text-coffee hover:underline"
              >
                Open lineage →
              </button>
            )}
