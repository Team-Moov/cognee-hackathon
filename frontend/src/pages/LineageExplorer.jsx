...
const NODE_STYLES = {
  hypothesis: {
    bg: "bg-coffee/8 border-coffee/30",
    accent: "bg-coffee",
    label: "HYPOTHESIS",
    text: "text-coffee-deep",
  },
  decision: {
    bg: "bg-ochre/8 border-ochre/30",
    accent: "bg-ochre",
    label: "DECISION",
    text: "text-ochre",
  },
  config: {
    bg: "bg-sand border-line",
    accent: "bg-muted",
    label: "CONFIG",
    text: "text-cocoa",
  },
  result: {
    bg: "bg-olive/8 border-olive/30",
    accent: "bg-olive",
    label: "RESULT",
    text: "text-olive",
  },
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <span
          className={`mt-0.5 h-8 w-1 flex-shrink-0 rounded-full ${style.accent}`}
        />
        <div className="min-w-0 flex-1">
          <div
            className={`mb-1 text-xs font-bold uppercase tracking-wider ${style.text}`}
          >
            {style.label}
          </div>
          ...
          {d.rationale && (
            <div className="mt-1 text-xs italic text-muted">
              {expanded
                ? d.rationale
                : `${d.rationale.slice(0, 80)}${d.rationale.length > 80 ? "…" : ""}`}
            </div>
          )}
          ...
          {expanded && (
            <div className="mt-2 space-y-1">
              ...
              {d.made_by && (
                <div className="text-xs text-muted">by {d.made_by}</div>
              )}
              {d.gpu_hours && (
                <div className="text-xs text-muted">{d.gpu_hours}h GPU</div>
              )}
            </div>
          )}
        </div>
...
export default function LineageExplorer() {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl p-6 sm:p-8 lg:px-10">
      ...
      <PageHeader title="Lineage Explorer" subtitle={runId} />
      ...
      {error && (
        <div className="py-12 text-center text-sm text-terracotta">
          Error: {error}
        </div>
      )}
      ...
      {lineage && (
        <div className="mt-6 text-center text-xs text-muted">
          {lineage.nodes.length} nodes · {lineage.edges.length} edges · Click
          any card to expand
        </div>
      )}
    </div>
